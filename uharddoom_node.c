			...
			struct uharddoom_pagedir *pagedir)
{
	...
	printk(KERN_ALERT "init_pagedir - data_cpu %x\n", pagedir->data_cpu);
	return 0;
}
	...
	unsigned last)
{
	...
	for (i = start + skip_start; i <= last - skip_last; ++i) {
		...
		*pagedir_entry_p = (entry->data_dma >> UHARDDOOM_PDE_PA_SHIFT)
		...
		// printk(KERN_ALERT "allocate_pde_range - linked pagetable %u, entry_addr: %x\n",
			// i, pagedir_entry_p);
		pagedir_entry_p++;
	}
	...
	unsigned readonly)
{
	...
	printk(KERN_ALERT "buffer_fill_pagetables - start_addr: %x, last_addr: %x\n",
	...
	list_for_each(pos, &pagedir->pagetables) {
		entry = list_entry(pos, struct uharddoom_pagetable, lh);
		if (entry->idx == start_pdi) {
			printk(KERN_ALERT "buffer_fill_pagetables - found entry idx %x\n",
				start_pdi);
			break;
		}
	}
	...
	printk(KERN_ALERT "buffer_fill_pagetables - initial pagetable entry_addr: %x\n",
		pagetable_entry_p);
	...
	for (i = 0; i < page_count; ++i) {
		// printk(KERN_ALERT "buffer_fill_pagetables - page %u\n",
			// i);
		// printk(KERN_ALERT "buffer_fill_pagetables - start_pdi: %x, start_pti: %x, entry_addr: %x\n",
...
static int add_mapping(struct uharddoom_context *ctx, struct fd file,
	...
	unsigned page_count, unsigned readonly)
{
	...
	printk(KERN_ALERT "add_mapping - start_addr: %x, last_addr: %x, page_count: %u\n",
		start_addr, last_addr, page_count);
...
static long map_buffer(struct file *filp, unsigned buf_fd, unsigned readonly)
{
	...
	ret = add_mapping(
		ctx, b_file, pos, prev_end << UHARDDOOM_PAGE_SHIFT,
		page_count, readonly
	);
	if (ret < 0)
		goto out_fdput;
